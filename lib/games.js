...
Meteor.methods({
    ...
    'games.changeTurns' : function (id) {
        ...
        // TODO: Must change this to match new paradigm of movePhase and isFinishedWithCards and isFinishedWithMove
        ...
        game.players[0].state.canSelectTiles = !game.players[0].state.canSelectTiles;
        game.players[1].state.canSelectTiles = !game.players[1].state.canSelectTiles;
     ...
     * @param id
     */
    'games.setFinishedWithTurn' : function (value, id, player) {
        // check if this user has the authority to do so
        let game = Games.findOne({id: id});

        game.players[player].state.finishedWithTurn = value;
        Games.update({id: id}, {$set: {players: game.players}});
    },
